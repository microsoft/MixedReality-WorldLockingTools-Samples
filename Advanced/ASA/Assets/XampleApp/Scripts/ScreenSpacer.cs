...
public class ScreenSpacer : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        Pose globalPose = cam.transform.GetGlobalPose().Multiply(localPose);
        t.SetGlobalPose(globalPose);
    }
...
#if false
        Debug.Log($"Distance={distance}, w={width}, h={height}, y={offsetHeight}, fov={(adjustedFOV.x * Mathf.Rad2Deg).ToString("0.0")},{adjustedFOV.y * Mathf.Rad2Deg}");
        Rect pr = CameraCache.Main.pixelRect;
        Debug.Log($"R={pr.x},{pr.y},{pr.x + pr.width}, {pr.y + pr.height}");
#endif
#if false
        if (Mathf.Approximately(0, 0.0f) == false)
        {
            Debug.LogError("Mathf.Approximately is failing somehow");
        }
#endif
